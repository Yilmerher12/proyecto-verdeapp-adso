...
`routers/` (endpoints FastAPI, un archivo por dominio) → `services/` (lógica de negocio, cuando el router hace más que un CRUD simple) → `models/` (SQLAlchemy ORM) + `schemas/` (Pydantic, request/response). `dependencies.py` tiene las dependencias inyectables que usa casi todo endpoint: `get_db` (sesión de BD), `get_current_user` (valida el JWT), `require_role(RolId.X, mensaje)` (exige un rol) y `require_admin_conjunto(mensaje)` — esta última devuelve el `AdministradorConjunto` ya resuelto, no el `Usuario`, así que el router usa `administrador.id_usuario` al llamar al servicio. Para restringir un endpoint por rol se usan estas dos, no un `if current_user.id_rol != ...` a mano.
...
- **Notificaciones**: toda notificación nueva se crea con `crear_notificacion(db, tipo=..., mensaje=..., destinatarios=...)` de `app/services/notificaciones_helpers.py`, no armando `Notificacion(...)` a mano. El helper crea la fila SIEMPRE, aunque `destinatarios` esté vacío — si un servicio no quiere crear una notificación sin destinatarios, revisa `if not destinatarios: return` él mismo antes de llamarlo. `enviar_notificacion` (SHUT lleno/libre) NO debe hacer esa revisión: su estado se lee de la fila creada, no de quién la recibió.
- **Paginación**: los listados que pueden crecer reciben `limit` y `offset` como `Query`, el servicio devuelve `(items, total)` y el router responde `{"items": ..., "total": ...}`. En el frontend se consume con el hook `usePaginacion` y el componente `<Paginacion>` (ver `AdminConjuntoComunicadosPage.tsx` como ejemplo).
- **Base de datos de pruebas**: `pytest` usa una BD aparte, `verdeapp_test_db` (`be/app/tests/conftest.py`), que `docker compose up` NO crea (solo crea `verdeapp_db`) y que un reinicio del volumen también borra. Se crea una sola vez con `docker exec verde_db psql -U verde_user -d verdeapp_db -c "CREATE DATABASE verdeapp_test_db OWNER verde_user;"` — las tablas las crea `pytest` solo. Si toda la suite falla con `database "verdeapp_test_db" does not exist`, es esto.
...
- **Componentes compartidos — reutilizarlos, no copiar el patrón a mano**: `EmptyState` para toda lista vacía (mensaje con la fórmula "Todavía no hay/tienes X..."); `ConfirmModal` para toda acción destructiva o que pida confirmación (prop `layer="stacked"` si va encima de otro modal); `Alert` para avisos de error/éxito; `InputField` para campos de texto (no soporta `<select>` ni `<textarea>` — esos se dejan crudos, pero con el mismo `onBlur`/`aria-invalid`/`aria-describedby` y párrafo `role="alert"`).
- **Validación de formularios**: por campo al salir de él (`onBlur`), con el mensaje anclado a ese campo vía `error` de `InputField` (ver `RegisterPage.tsx`); el submit revisa todos los campos juntos. El `Alert` genérico queda solo para fallos reales de guardado o de red, no para "falta un campo".
- **Fechas**: formatear siempre con `lib/dateFormat.ts` (`formatearFechaCreacion` para instantes como `created_at`, `formatearFechaUTC` para fechas elegidas a mano como `fecha_expiracion`), nunca con `new Date(x).toLocaleDateString()` suelto.
...
- **Los comandos que se le dan al usuario para la terminal deben ser simples: uno por bloque, sin bucles (`for`), pipes (`|`) ni one-liners con `grep`/`$(...)`** salvo que se expliquen paso a paso antes. El usuario está aprendiendo la terminal a la vez que el proyecto; si hace falta repetir una operación sobre varios elementos (ej. borrar varias ramas), listar primero los nombres con un comando de solo lectura y dar un comando por elemento, o explicar el bucle pieza por pieza antes de pedirle que lo corra.
- **Antes de proponer el alcance de un issue del tablero, comprobar en el código que sigue vigente**: los issues se escriben en un momento y el código sigue cambiando (ya pasó con "deshabilitar el botón Llegué al conjunto" y con parte de "corregir 5 documentos de requisitos", que ya estaban resueltos). Si ya está resuelto, decirlo y proponer cerrar el issue en vez de implementarlo de nuevo.
- **Un "error de CORS" en el navegador no siempre es CORS**: si el login (u otro endpoint) falla con `No 'Access-Control-Allow-Origin' header`, revisar primero `docker ps` (¿`verde_db` está `healthy`?) y los logs del backend — un 500 por BD caída o por una función SQL rota sale del backend sin cabeceras CORS y el navegador lo reporta como bloqueo CORS.
...
- **Si una tarjeta agrega o modifica una migración de Alembic**, dar siempre el comando exacto para que el usuario actualice su propia base de datos (`uv run alembic upgrade head`, desde `be/`), igual que se dan los comandos de git — no asumir que su BD ya quedó al día solo porque la migración existe en el código. Además, si durante la implementación o la verificación se corrió esa migración (u otro cambio manual, como aplicar `alembic upgrade head`, sembrar datos sueltos, o insertar/editar filas a mano) directamente contra la base de datos de desarrollo, decirlo explícitamente y dar los comandos para reiniciar el volumen de Docker de la BD antes de que el usuario siga usando el proyecto:
  ...
  ```
  esperar a que `docker ps` muestre `verde_db` como `healthy` (con el volumen nuevo Postgres tarda unos segundos en iniciar; si se corre el siguiente comando antes, falla con `connection to server ... failed`), recrear la BD de pruebas (el reinicio también borra `verdeapp_test_db`, ver Arquitectura):
  ```bash
  docker exec verde_db psql -U verde_user -d verdeapp_db -c "CREATE DATABASE verdeapp_test_db OWNER verde_user;"
  ```
  y luego, DESDE `be/` (desde la raíz `uv` no encuentra `alembic`), aplicar migraciones + seed desde cero (`uv run alembic upgrade head` y `uv run python -m app.seed`). El objetivo es que el estado final de la BD sea siempre el que resultaría de que cualquier integrante del equipo clone el repo y siga los pasos del README — nunca un estado que solo existe porque Claude lo dejó así en su propia sesión de pruebas.
