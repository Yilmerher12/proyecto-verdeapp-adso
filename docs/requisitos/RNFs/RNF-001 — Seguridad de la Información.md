...
- **Rate limiting realmente conectado**: existía el límite de intentos de login/registro (`slowapi`) pero nunca se había registrado con FastAPI — superarlo no daba una respuesta 429 real. Corregido y verificado contra el servidor real.
  - **Issue #310 (hallazgo CN-013 del informe Cyber Neo)**: el límite solo cubría `/register` (5/min), `/login` (10/min) y `/forgot-password` (5/min). Se agregó a `/change-password`, `/reset-password`, `/verify-email` y `/admin-conjunto/aceptar` (5/min; sin límite permitían adivinar la contraseña actual o probar tokens al azar), y a `/refresh` y `/uploads/adjunto` (30/min). Pendiente para un despliegue real (issue #317): guardar los contadores en Redis y correr uvicorn con `--proxy-headers`; hoy viven en memoria y detrás de un proxy todos los usuarios compartirían la IP del proxy.
...
- **Issue #311 (2026-09-23):** `POST /admin-conjunto/aceptar` era el único endpoint que todavía devolvía `access_token`/`refresh_token` en el cuerpo JSON (hallazgo CN-028 del informe Cyber Neo). Como el frontend nunca los usaba (la pantalla manda a iniciar sesión), se quitaron: ahora responde solo un mensaje. Ningún endpoint entrega tokens fuera de las cookies httpOnly.
